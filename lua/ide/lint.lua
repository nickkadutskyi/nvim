...
M.linters = {
    ...
    phpmd = { ---@type ide.Linter
        ...
        parser = function(output, _)
            ...
            local ok
            ok, output = I.phpmd.strip_deprecations(output)
...
function I.is_match(line, patterns)
...
function I.phpmd.strip_deprecations(output)
    -- quick check: if we see a fatal pattern anywhere, give up early
    for line in output:gmatch("[^\r\n]+") do
        if I.is_match(line, I.php.FATAL_PATTERNS) then
    ...
    local cleaned = {}
    for line in output:gmatch("[^\r\n]+") do
        if not I.is_match(line, I.php.IGNORE_PATTERNS) then
